...
```{r Bilirubin_LDH_bothparts, fig.width=12, fig.height=8}
...
plot(xx1$Time, xx1$Tot_bili_absolute,
     ...
     xlab = 'Days', main = 'Part 1',ylim = ylims,
...
plot(xx2$Time, xx2$LDH_absolute, xlim=c(0,14), panel.first = grid(),
     xlab = 'Days', main = 'Part 2',yaxt='n',ylim = ylims,
     ylab = 'Absolute change in LDH (U/L)')
for(id in unique(xx2$label)){
  ind=xx2$label==id
...
plot(xx1$Day10_total_PQ, xx1$Max_TBR_increase, 
     panel.first = grid(),ylim=ylims,
     xlab = 'Day 10 cumulative dose (mg/kg)', main = 'Part 1',
...
plot(PQ_unique$Day10_total_PQ, PQ_unique$Max_hb_fall_abs,
     ...
     ylab = 'Maximum absolute reduction (Hb, g/dL)',
...
plot(PQ_unique$Day10_total_PQ, PQ_unique$Max_hb_fall_rel,
     xlim = c(1.5, 3.5), xlab = "Day 10 cumulative dose (mg/kg)",
     ylab = 'Maximum relative reduction (Hb, %)', panel.first = grid(),
...
for(my_col in c('Max_hb_fall_abs','Max_hb_fall_rel')){
  ...
  plot(PQ_unique$Total_PQ, t(PQ_unique[,my_col]),
       ...
       ylab=ifelse(my_col=='Max_hb_fall_abs',
                   'Maximum absolute reduction (Hb, g/dL)',
                   'Maximum relative reduction (Hb, %)'),
       xlab = "Total PQ dose (mg/kg)",col='darkblue')
   if(my_col=='Max_hb_fall_abs'){
    mtext(text = 'Absolute reduction',side = 3,line = 1.5,adj = 0)
  } else {
    mtext(text = 'Relative reduction',side = 3,line = 1.5,adj = 0)
  }
...
```{r individ_profiles_part1_2, fig.width=10, fig.height=6}
...
```{r methaemoglobin_primaquine}
# par(mfrow=c(3,3),las=1, cex.axis=1.3, cex.lab=1.3)
# for(id in unique(PQdat$label)){
#   xx = PQdat %>% filter(label==id, !is.na(lbmethb))
#   plot(xx$Time, xx$lbmethb, xlim = c(0,28), type='b', ylim=c(0,2.4), panel.first=grid(),
#        xlab='Time', ylab='Blood methaemoglobin (%)')
# }
...
par(las=1, mfrow=c(2,2), family='serif', cex.lab=1.3, cex.axis=1.3)
# AST - normalised by baseline
plot(xx$Day10_total_PQ, xx$Max_AST_increase,
     xlab = 'Day 10 cumulative dose (mg/kg)',pch=16,
     ylab = 'Maximum AST (fold change)',
     yaxt='n', panel.first=grid())
axis(2, at = -1:3, labels = 2^(-1:3))
mod = lm(Max_AST_increase ~ Day10_total_PQ, data=xx)
summary(mod); abline(mod)
# ALT
plot(xx$Day10_total_PQ, xx$Max_ALT_increase,
     xlab = 'Day 10 cumulative dose (mg/kg)', pch=16,
     ylab = 'Maximum ALT (fold change)',
     yaxt='n', panel.first=grid())
axis(2, at = -1:4, labels = 2^(-1:4))
mod = lm(Max_ALT_increase ~ Day10_total_PQ, data=xx)
summary(mod); abline(mod)

# Creatinine
plot(xx$Day10_total_PQ, xx$Max_Creat_increase,
     xlab = 'Day 10 cumulative dose (mg/kg)', pch=16,
     ylab = 'Maximum creatinine (fold change)',
     yaxt='n', panel.first=grid())
axis(2, at = c(-0.4, -.2, 0 , .2, .4), labels = round(2^c(-0.4, -.2, 0 , .2, .4),2))
mod = lm(Max_Creat_increase ~ Day10_total_PQ, data=xx)
summary(mod); abline(mod)


## Haptoglobin
plot(xx$Day10_total_PQ, xx$Max_Haptoglobin_decrease,
     xlab = 'Day 10 cumulative dose (mg/kg)', pch=16,
     ylab = 'Haptoglobin (absolute change, g/L)',
     panel.first=grid())
mod = lm(Max_Haptoglobin_decrease ~ Day10_total_PQ, data=xx)
summary(mod); abline(mod)
...
PQ20 = PQdat %>% filter(label=='ADPQ 20', Day>=0, Day <=20) %>%
  group_by(Day) %>%
  mutate(PQdose = sum(PQdose),
         Haemocue_hb = mean(Haemocue_hb, na.rm=T),
         Manual_retic = mean(Manual_retic, na.rm=T),
...
PQ15 = PQdat %>% filter(label=='ADPQ 15', Day>=0, Day <=20) %>%
  group_by(Day) %>%
  mutate(PQdose = sum(PQdose),
         Haemocue_hb = mean(Haemocue_hb, na.rm=T),
         Manual_retic = mean(Manual_retic, na.rm=T),
         CBC_hb = mean(CBC_hb, na.rm = T))
par(mar=c(0,5,2,2))
plot(PQ15$Time, PQ15$PQdose/PQ15$weight,ylim = c(0,1),
     ylab='Primaquine (mg/kg)',xlab='',xaxt='n',type='l',lwd=3,
     panel.first = grid())
text(x = 7, y=0.85, 'V15: 6.8mg/kg over 15 days', cex=1.3)

par(mar=c(1,5,1,2))
plot(PQ15$Time, PQ15$Haemocue_hb,ylim=c(9,16),
     xlab='', ylab = 'Haemoglobin (g/dL)', 
     panel.first = grid(), pch=16, cex=1.5, xaxt='n')
points(PQ15$Time, PQ15$CBC_hb,
       pch=15, cex=1.5, col='red')
par(mar=c(5,5,1,2))
plot(PQ15$Time, PQ15$Manual_retic,ylim=c(0,16),
     xlab='Day since starting PQ', ylab = 'Reticulocytes (%)', 
     panel.first = grid(), pch=16, cex=1.5)
