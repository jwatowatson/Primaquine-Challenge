...
write_csv(PQdat, file = 'Data/PQdat.csv')
...
for(my_col in c('Max_hb_fall_abs','Max_hb_fall_rel')){
  ...
  plot(PQ_unique$Total_PQ, t(PQ_unique[,my_col]),
       ...
       xlab = "Total primaquine dose (mg/kg)",col='darkblue')
...
for(my_p in c('Part1', 'Part2')){
  ...
  xs = c(1,2,2,3,3)
  layout(mat = matrix(data = c(xs, xs+3, xs+6),nrow = 5,byrow = F))
  if(my_p=='Part1'){my_x_lim=c(0,28)} else {my_x_lim=c(0,14)}
  for(id in unique(PQdat$label[PQdat$study==my_p])){
    ...
    par(mar = c(0,5,4,2))
    plot(PQdat_id$Day, PQdat_id$PQdose_daily/PQdat_id$weight,
         xlab='', ylim=c(0,1),yaxt='n',panel.first=grid(),
         type='l', ylab='mg/kg',xaxt='n',xlim=my_x_lim,
         main=paste0(id, ' (',PQdat_id$G6PD_variant[1],')'))
    axis(2, at =c(0,0.5,1))
    text(x = 7, y=0.85, paste0(round(max(PQdat_id$PQ_mg_kg_cum),1),' mg/kg total'), cex=1.3)
...
plot(PQ_11$Time, PQ_11$PQdose/PQ_11$weight,ylim = c(0,1),
     ylab='Primaquine (mg/kg)',xlab='',xaxt='n',type='l',lwd=3,
     panel.first = grid(), yaxt='n')
axis(2, at =c(0,.5,1))
text(x = 7, y=0.85, paste0(round(max(PQdat_id$PQ_mg_kg_cum),1),' mg/kg total'), cex=1.6)
...
par(mar=c(0,5,2,2))
plot(PQ_11$Time, PQ_11$PQdose/PQ_11$weight,
     ylab='Primaquine (mg/kg)',xlab='',xaxt='n',type='l',lwd=3,ylim=c(0,1),
     panel.first = grid())
text(x = 7, y=0.85, paste0(round(max(PQdat_id$PQ_mg_kg_cum),1),' mg/kg total'), cex=1.6)
par(mar=c(5,5,0,2))
...
PQ20 = PQdat %>% filter(label=='ADPQ 20', Day <=20, Day>=0) %>%
  group_by(Day) %>%
  mutate(PQdose = sum(PQdose),
         Haemocue_hb = mean(Haemocue_hb, na.rm=T),
         Manual_retic = mean(Manual_retic, na.rm=T),
...
##### ************* Volunteer 20 **************
plot(PQ20$Time, PQ20$PQdose/PQ20$weight,ylim = c(0,1),yaxt='n',
     ylab='Primaquine (mg/kg)',xlab='',xaxt='n',type='l',lwd=3,
     panel.first = grid())
axis(2, at =c(0,.5,1))
abline(v=0)
text(x = 7, y=0.85, 'V20: 5.5 mg/kg over 15 days', cex=1.3)
...
##### ************* Volunteer 15 **************
PQ15 = PQdat %>% filter(label=='ADPQ 15',Day <=20, Day >= 0) %>%
  group_by(Day) %>%
  mutate(PQdose = sum(PQdose),
         Haemocue_hb = mean(Haemocue_hb, na.rm=T),
         Manual_retic = mean(Manual_retic, na.rm=T),
...
par(mar=c(0,5,2,2))
plot(PQ15$Time, PQ15$PQdose/PQ15$weight,ylim = c(0,1),xlim = c(0,20),
     ylab='Primaquine (mg/kg)',xlab='',xaxt='n',type='l',lwd=3,
     panel.first = grid(), yaxt='n')
text(x = 7, y=0.85, 'V15: 6.8 mg/kg over 15 days', cex=1.3)
abline(v=0,lty=2)
axis(2, at =c(0,.5,1))

par(mar=c(0,5,0,2))
plot(PQ15$Time, PQ15$Haemocue_hb,ylim=c(9,16),xlim = c(0,20),
     xlab='', ylab = 'Haemoglobin (g/dL)', 
     panel.first = grid(), pch=16, cex=1.5, xaxt='n')
...
par(mar=c(5,5,0,2)) 
plot(PQ15$Time, PQ15$Manual_retic,ylim=c(0,16),xlim = c(0,20),
     xlab='Days since starting PQ', ylab = 'Reticulocytes (%)', 
     panel.first = grid(), pch=16, cex=1.5)
