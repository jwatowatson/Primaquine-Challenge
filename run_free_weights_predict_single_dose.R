...
#
# Fit the RBC model with independent dose-weighting to the ascending-dose
# study, and predict the effect of the single-dose regimens.
#
...
main <- function(args) {
  ...
  jobs <- utils$create_job_data_ascending_dose_predict_single_dose(
  ...
  for (job_ix in seq_along(jobs)) {
    ...
    results_file <- file.path(
      "Rout", paste0(prefix, "_predict_single_dose_", job_ix, ".rds")
    )
...
call_main("run_free_weights_predict_single_dose.R", main)
